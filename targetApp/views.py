        ...
        try:
            ...
            # Import from txt / csv
                ...
                if txt_file:
                    ...
                    is_csv = csv_file.content_type = (
                        "text/csv" or csv_file.name.split(".")[-1] == "csv"
                    )
                    if not is_csv:
                        messages.add_message(
...
def list_target(request, slug):
    context = {"list_target_li": "active", "target_data_active": "active", "slug": slug}
...
def update_target(request, slug, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.add_message(
                request, messages.INFO, f"Domain {domain.name} modified!"
            )
            return http.HttpResponseRedirect(
...
def list_organization(request, slug):
    organizations = Organization.objects.filter(project__slug=slug).order_by(
        "-insert_date"
    )
