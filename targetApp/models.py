...
class DomainInfo(models.Model):
    ...
    related_domains = models.ManyToManyField(
        RelatedDomain, blank=True, related_name="associated_domains"
    )
    related_tlds = models.ManyToManyField(
        RelatedDomain, blank=True, related_name="related_tlds"
    )
    similar_domains = models.ManyToManyField(
        RelatedDomain, blank=True, related_name="similar_domains"
    )
    # historical ips
    historical_ips = models.ManyToManyField(
        HistoricalIP, blank=True, related_name="similar_domains"
    )
...
class Organization(models.Model):
    ...
    def get_domains(self):
        return Domain.objects.filter(
            domains__in=Organization.objects.filter(id=self.id)
        )
