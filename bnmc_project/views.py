...
from .serializers import (Education_info, PostSerializer, Student_info,
                          sliderSerializer)
...
def examination_result_add(request, main_subject):
    ...
    if not request.user.is_superuser:
        get_request_user = UserPermissionResult.objects.filter(
            user=request.user.id
        ).last()
        exam_student_reg = ExaminationStudentRegistration.objects.filter(
            exam=get_request_user.final_exam,
            roll_number__range=(get_request_user.start_roll, get_request_user.end_roll),
        )
        ...
        for students in sort_list:
            ...
            for user_subject in get_request_user.sub_subjects.all():
                filter_result_add = Examination_result_add.objects.filter(
                    exam_id=students.id, subject=user_subject.id
                )
                ...
                if filter_result_add:
                    ...
                    set_mark_and_id = (
                        str(user_subject.id) + "-" + str(user_subject.marks)
                    )
                    result_dimention_list[start_zero].append(set_mark_and_id)

                if filter_result_add:
                    if (
                        filter_result_add[0].mark
                        > filter_result_add[0].subject.pass_marks
                        or filter_result_add[0].mark
                        == filter_result_add[0].subject.pass_marks
                    ):
                        status = "Pass"
        ...
        context = {
            "own_students": zip(
                sort_list, result_dimention_list, pass_status_dimention_list
            ),
            "sub_main": sub_main,
    ...
    if request.method == "POST":
        ...
        for obtain, student_id, sub_subject_id in zip(
            obtain_mark_list, exam_student_list, sub_subject_list
        ):
            print(obtain, student_id, sub_subject_id)
            result_filter = Examination_result_add.objects.filter(
                exam_id=student_id, subject=sub_subject_id
            )
            ...
            if result_filter:
                ...
                if obtain != "":
                    exam_student_filter = ExaminationStudentRegistration.objects.get(
                        id=student_id
                    )
...
def division_wise_report(request):
    ...
    context = {"division": division, "program": program, "session": session}
    ...
    if request.method == "POST":
        ...
        for n in district_list:
            ...
            for ins in institutions:
                student_approve_total = Student_Registration.objects.filter(
                    session=session,
                    program_title=program,
                    approved=True,
                    institution__institution_name=ins,
                ).count()
        ...
        try:
            if division != 0 and program != 0 and session != 0:
                ...
                response[
                    "Content-Disposition"
                ] = 'attachment; filename="exam_wise_report_for_bnmc.xls"'
                ...
                for d in district_list:
                    ...
                    for p, total in zip(dimension_list[zero], student_total[zero]):
                        ...
                        ws.write(
                            row_num,
                            4,
                            d + " total students " + str(sum(student_total[zero])),
                            font_style,
                        )
...
def new_end_date_implement(date_end_inc):
    if date_end_inc.day == 1:
        ...
        new_end_date_implement = date(
            date_end_inc.year + 5, date_end_inc.month, date_end_inc.day - 1
        )
...
def license_select(request):
    ...
    for i in license_rec_list:
        ...
        if len(search_licenseHis_q) == 1:
            ...
            if (
                search_licenseHis_q.program
                and search_licenseHis_q.license_number == None
            ):
                search_by_program = LicenseHistory.objects.filter(
                    program=search_licenseHis_q.program.id
                ).aggregate(Max("license_number"))
...
def apply_for_renew(request):
    if "data" in request.POST:
        ...
        license_his = LicenseHistory.objects.filter(
            license_number=license_number, program=program_id
        )
        if license_his:
            license_query = LicenseHistory.objects.filter(
                license_receive_id=license_his[0].license_receive_id
            )
            ...
            for i in license_query:
                ...
                if i.license_end_date:
                    ...
                    if current_date > expire_date:
                        req_re = RequestedLicense.objects.filter(
                            program=i.program,
                            license_number=i.license_number,
                            name=i.license_receive_id.last_name,
                        )
                        if req_re:
        ...
        I_always_wear_my_professional = request.POST.get(
            "I_always_wear_my_professional"
        )
        ...
        apply_save = ApplyLicense(
            I_am_satisfied=I_am_satisfied,
            before_starting=before_starting,
            I_own_my=I_own_my,
            I_am_interested=I_am_interested,
            I_am_aware=I_am_aware,
            I_am_aware_of_prescribed=I_am_aware_of_prescribed,
            Every_day=Every_day,
            In_Bangladesh_most=In_Bangladesh_most,
            I_always_try=I_always_try,
            ...
            I_feel_there_should_be=I_feel_there_should_be,
            I_know_the_number=I_know_the_number,
            urgent=urgent,
            currently_working_place=current_working_place,
            current_position=current_position,
            Working_years_as_a_registered=working_years,
            Training=traning,
            Attendance_in_Seminar=a_s,
            Research_skills=s_o,
        ...
        context = {"program": Program.objects.all(), "suc": "Your apply successful"}
...
def get_data(request):
    ...
    license_find = LicenseHistory.objects.filter(
        license_receive_id=license_his[0].license_receive_id
    )
    ...
    if request.method == "POST":
        if "get_data" in request.POST:
            ...
            if is_nurse or student_id:
                institute_save = IntuitionProfile(
                    is_nurse=True,
                    ...
                    date_of_birth=None,
                    license_number=None,
                    students_mobile_no=None,
                    institute_name=institute,
                    employment_type=1,
                    work_starting_date=w_start_date,
                    faculty_id=f_id,
                    image=None,
                ...
                if (
                    ...
                    institute_save = IntuitionProfile(
                        ...
                        students_mobile_no=no,
                        institute_name=institute,
                        employment_type=2,
                        license_number=license_number,
                        work_starting_date=w_start_date,
                        faculty_id=f_id,
                        image=str("/institute_profile/" + file),
                        designation=designation_obj,
                    )
...
def search_with_license_number(request):
    if request.method == "POST":
        ...
        if number and program:
            license_qr = LicenseHistory.objects.get(
                license_number=number, program=program
            )
            if license_qr:
                list_of_collections = []

                license_history_qr = LicenseHistory.objects.filter(
                    license_receive_id=license_qr.license_receive_id
                )
                current_date = datetime.datetime.now().date()
                ...
                context = {
                    "qr": zip(license_history_qr, list_of_collections),
                    "info_student": license_history_qr[0],
                    "program_objs": Program.objects.all(),
                }
                print(list_of_collections)
                return render(request, "admin/license_number_search.html", context)

    return render(
        request,
        "admin/license_number_search.html",
        {"program_objs": Program.objects.all()},
    )
...
def license_card_info(request, entry):
    ...
    for i in license_his_qr:
        ...
        if current_date > expire_date:
            ...
            list_of_collections.append("Renewed")
    context = {
        "qr": zip(license_his_qr, list_of_collections),
...
def edit_final_center(request):
    ...
    if center_query:
        info = [
            True,
            center_query.id,
            center_query.exam.id,
            center_query.roll_start,
            center_query.roll_end,
            center_query.center.id,
        ]
        print(info)
    else:
        info = [False]
...
def report_of_center_for_final_exam(request, center_id):
    ...
    exam = ExaminationStudentRegistration.objects.filter(
        roll_number__range=(center.roll_start, center.roll_end),
        center=center.center,
        exam=center.exam,
    ).order_by("roll_number")
    get_program = ExaminationStudentRegistration.objects.filter(
        roll_number__range=(center.roll_start, center.roll_end),
        center=center.center,
        exam=center.exam,
    ).last()
    exam_subs = ExamSubject.objects.filter(program=get_program.program.id).order_by(
        "id"
    )
...
def set_center(request):
    if request.POST:
        ...
        if exam and start_role and end_role and center:
            exam_students = ExaminationStudentRegistration.objects.filter(
                roll_number__range=(start_role, end_role), approved=True, exam__id=exam
            )
    ...
    context = {
        "institution_name": centers_institution,
        "exams": Final_exam.objects.all(),
        "var": paginate,
    }
    ...
    if last_permission.id == permission_id and student_reg.approved == False:
        ...
        while is_valid == False:
            ...
            auto_reg_no = ExaminationStudentRegistration.objects.filter(
                program__code=student_reg.program.code, roll_number=reg_no
            )
...
def final_exam_wise_report(request):
    exam = Final_exam.objects.all()
    program = Program.objects.all()
    context = {"exam": exam, "program": program}
    ...
    if request.method == "POST":
        ...
        try:
            if exam_id != 0 and program != 0:
                exam_students = ExaminationStudentRegistration.objects.filter(
                    exam=exam_id, program=program_id
                )
                if exam_students:
                    total = 0
                    response = HttpResponse(content_type="application/ms-excel")
                    response[
                        "Content-Disposition"
                    ] = 'attachment; filename="Final_exam_wise_report_for_bnmc.xls"'
...
def signature_upload_principal(request):
    request_user_institution = request.user.staff_institute.values_list(
        "id", flat=True
    )[0]
...
def unlock_history(request, id_history):
    if not request.user.is_superuser:
        ...
        license_receive_id = license_receive.objects.get(
            id=lic_his_id.license_receive_id.id
        )
        if lic_his_id:
        ...
        return redirect(
            "/admin/bnmc_project/license_receive/"
            + ""
            + str(license_receive_id.id)
            + ""
            + "/change/",
        )
...
def print_license_card(request, license_history_card):
    ...
    if lic_his_id.lock == False:
        ...
        lic_his_id.save()
        if (
            lic_his_id.card_serial is None
            or lic_his_id.card_serial == 0
            or lic_his_id.card_serial == ""
        ):
            ...
            for key, value in code.items():
                if key == lic_his_id.program.code:
                    ...
                    while is_found == False:
                        last_no = LicenseHistory.objects.filter(
                            card_serial=card_no, program__code=lic_his_id.program.code
                        )
                        if last_no:
                            card_no += 1
        ...
        if lic_his_id.license_registration_date:
            ...
            full_license_registration_date = (
                zero + li_reg_day + "-" + zero_month + li_reg_month + "-" + li_reg_year
            )
        ...
        if lic_his_id.license_start_date:
            ...
            full_license_start_date = (
                zero + li_sta_day + "-" + zero_month + li_sta_month + "-" + li_sta_year
            )
        ...
        if lic_his_id.license_end_date:
            ...
            full_license_end_date = (
                zero + li_end_day + "-" + zero_month + li_end_month + "-" + li_end_year
            )
    ...
    if request.POST:
        ...
        with open(pwd + "/bnc_rnm_info_.csv", encoding="ISO-8859-1") as f:
            ...
            for row in reader:
                ...
                institute = None
                if institution_name:
                    ins = Institution.objects.filter(
                        institution_second_code=institution_name
                    )
                    if ins:
                        institute = ins[0]
                ...
                nationality = Nationality.objects.filter(
                    nationality_name__icontains=nationality_name
                )
                if nationality:
                ...
                if relation_to_guardian:
                    ...
                    relation_to_guardian = Relation_to_guardians(
                        relation=relation_with_guardians
                    )
                ...
                try:
                    bank_draft_date = datetime.datetime.strptime(
                        bank_draft_date, "%d-%m-%Y"
                    ).date()
                except ValueError as e:
                ...
                try:
                    course_start_date = datetime.datetime.strptime(
                        course_start_date, "%d-%m-%Y"
                    ).date()
                except:
                    course_start_date = None

                try:
                    course_completion_date = datetime.datetime.strptime(
                        course_completion_date, "%d-%m-%Y"
                    ).date()
                except:
                ...
                try:
                    registation_date = datetime.datetime.strptime(
                        registation_date, "%d-%m-%Y"
                    ).date()
                except:
                    registation_date = None

                try:
                    month_info = datetime.datetime.strptime(
                        month_info, "%d-%m-%Y"
                    ).date()
                except:
                    month_info = None
                ...
                division_snd = Division.objects.filter(
                    name__icontains=division_snd_name
                )
                if not division_snd:
                ...
                district_snd = District.objects.filter(
                    name__icontains=district_snd_name
                )
                if not district_snd:
                    district_snd = District(
                        name=district_snd_name, division=division_snd
                    )
                    district_snd.save()
                else:
                ...
                if not thana_snd:
                    ...
                    try:
                        ...
                        stu = Student(
                            ...
                            relation_to_guardians=relation_to_guardian,
                            religions=religions_name,
                            marital_status=marital_status,
                            passport_no=passport_no,
                            students_mobile_no=students_mobile_no,
                            email_address=email_address,
                            division_snd=division_snd,
                            district_snd=district_snd,
                            thana_snd=thana_snd,
                            village_snd=village_snd,
                            postal_code_snd=postal_code_snd,
                        ...
                        if stu:
                            lic = license_receive(
                                ...
                                postal_code=postal_code,
                                approved=True,
                                status=student_permission,
                                is_old_data=True,
                                license_registration_fee=license_registration_fee,
                                payment_method=payment_method,
                                bank_draft_no=bank_draft_no,
                                registration_fee=registration_fee,
                                bank_draft_date=bank_draft_date,
                            ...
                            if lic:
                                license_history = LicenseHistory(
                                    ...
                                    institution=institute,
                                    student_id=stu,
                                    license_number=float(rnm_reg_no),
                                    registration_no=0,
                                    license_registration_date=final_start_date,
                                    license_start_date=final_start_date,
                                    license_end_date=final_end_date,
                                    license_receive_id=lic,
                                    lock=False,
...
def get_info_moneyrecipte(request):
    ...
    if number != 0 and pro != 0:
        student_reg = Student_Registration.objects.filter(
            program_title_id=pro, registration_no=number
        )
...
def get_info_moneyrecipte_li(request):
    ...
    if number != 0 and pro != 0:
        lic_reg = license_registrations.objects.filter(
            program_id=pro, rool_number=number
        )
        if lic_reg:
            lic_reg = lic_reg[0]
            if lic_reg:
                student_ = Student_Registration.objects.get(
                    id=lic_reg.student_registration.id
                )
                if student_:
...
def exam_wise_report(request):
    ...
    if request.method == "POST":
        ...
        try:
            if exam_id != 0 and program != 0:
                license_queryset = license_registrations.objects.filter(
                    exam_title=exam_id, program=program_id
                )
                ...
                if institition:
                    ...
                    for i in institition:
                        institution_wise_examinee = license_queryset.filter(
                            institution=i.id
                        )
                    ...
                    for i, total_number in zip(
                        institution_list, number_of_student__for_institution
                    ):
                        row_num += 1
                        no += 1
                        total += total_number
...
def report_of_center(request, center_id):
    ...
    license = license_registrations.objects.filter(
        rool_number__range=(center.roll_start, center.roll_end),
        centre=center.center,
        exam_title=center.exam,
    ).order_by("rool_number")
    print(license)

    context = {"license": license, "len": len(license), "center": center}
...
def get_license_rec_stu(request, licenserec_id):
    ...
    education_qulification = EducationQualification.objects.filter(
        students=stu_status.students.id
    )
    print(stu_status.id)
    context = {
        "license_id": stu_status,
...
class rest(FlatMultipleModelAPIView):
    querylist = [
        {
            "queryset": Student_Registration.objects.filter(id=4000),
            "serializer_class": Student_info,
        },
        {
            "queryset": EducationQualification.objects.filter(id=4000),
            "serializer_class": Education_info,
        },
    ]
...
def log(request):
    ...
    cotext = {"contacts": contacts, "acivity_users": User.objects.all()}
...
def changeExamStatus(request):
    ...
    if last_per.id == int(permission_id) and license_rec.approved == False:
        ...
        while is_run == False:
            license_number_set = license_receive.objects.filter(
    ...
    if last_permission.id == permission_id and student_reg.approved == False:
        ...
        while is_valid == False:
            ...
            auto_reg_no = Student_Registration.objects.filter(
                program_title__code=student_reg.program_title.code,
        ...
        if student_reg.students:
            ...
            if not student_reg.students.has_student_id:
                ...
                while end_loop == False:
                    search_student = Student.objects.filter(
...
def registration_step_li(request):
    ...
    if last_permission.id == permission_id and student_reg.approved == False:
        ...
        student_reg.approved = True
        while is_valid == False:
            auto_reg_no = license_registrations.objects.filter(
                program__code=student_reg.program.code, rool_number=student_reg_no
            )
            if auto_reg_no:
                student_reg_no += 1
            else:
...
def upload_file(request):
    context = {
        "exams": Exam.objects.filter(date_active=True),
        "exam": Exam.objects.all(),
    }
                ...
                for row in reader:
                    ...
                    try:
                        reg_no = license_registrations.objects.filter(
                            rool_number=int(registration), exam_title__id=exam_id
                        )
                        if reg_no:
...
def program_show(request):
    ...
    context = {"name": query}
    return render(
        request, "admin/bnmc_project/student_registration/change_list.html", context
    )
...
def get_registation_student_info(request):
    license_number = request.POST.get("registation_number")
    program_id = request.POST.get("program_id")
    license_info = LicenseHistory.objects.filter(
        license_number=license_number, program=int(program_id)
    )
    if license_info:
...
def get_registation_student_license(request):
    ...
    student_info = Student_Registration.objects.filter(
    ...
    if student_info:
        info = [
            ...
            student_info.fathers_name,
            student_info.mothers_name,
            student_info.students_mobile_no,
            student_info.sex,
            student_info.religions,
            student_info.nationality.id,
            student_info.division.id,
            student_info.district.id,
            student_info.thana.id,
            student_info.village,
            student_info.post_office,
            student_info.postal_code,
            str(student_info.image),
            str(student_info.image),
        ]
...
def info(request, student_id):
    ...
    context = {"student_registration": student, "education_qu": education_qu}
...
def search_place(request):
    search_key = request.POST.get("search_key").strip()
    place_list = []
    places = WorkingDetails.objects.filter(place__icontains=search_key).distinct(
        "place"
    )
    if places:
        for p in places:
...
def search_traning(request):
    search_key = request.POST.get("search_key").strip()
    place_list_ = []
    places = TrainingDetails.objects.filter(
        training_place__icontains=search_key
    ).distinct("training_place")
    if places:
        for p in places:
...
def search_post_office(request):
    ...
    Post_office_list = []
    print(search_key)
    post_offices = Student.objects.filter(post_office__icontains=search_key).distinct(
        "post_office"
    )
    if post_offices:
...
def search_snd_village(request):
    search_key = request.POST.get("search_key").strip()
    snd_village_list = []
    snd_villages = Student.objects.filter(village_snd__icontains=search_key).distinct(
        "village_snd"
    )
    if snd_villages:
...
def search_snd_post_office(request):
    ...
    snd_post_offices = Student.objects.filter(
        post_office_snd__icontains=search_key
    ).distinct("post_office_snd")
    if snd_post_offices:
...
def get_program_info(request):
    ...
    if program_id:
        ...
        if program:
            context = [
                True,
                program[0].session.session,
                float(program[0].duration.duration),
            ]
...
def Search_student(request):
    ...
    if query:
        ...
        context = {"data": data, "all_stu": all_student}
    return context
...
def add_student_info(request):
    if request.POST:
        ...
        with open(pwd + "/missing_student_list.csv", encoding="utf8") as f:
            ...
            approved_permission = Permission.objects.filter(
                permission_name="Approved BNMC"
            )
            if approved_permission:
                approved_permission = approved_permission[0]
            else:
                approved_permission = Permission(
                    permission_name="Approved BNMC", id_no=5, display_order=4
                )
            ...
            if draft_permissio:
                ...
                draft_permissio = Permission(
                    permission_name="Draft", id_no=1, display_order=0
                )
                draft_permissio.save()

            test_qualification = Qualification.objects.filter(
                minimum_qualification="Test qulification"
            )
            if test_qualification:
                test_qualification = test_qualification[0]
            else:
                test_qualification = Qualification(
                    minimum_qualification="Test qulification"
                )
            ...
            for row in reader:
                ...
                try:
                    ...
                    bank_draft_date_str = row["bankdraft_date"].strip()
                    approved = False
                    student_permission = Permission.objects.order_by(
                        "display_order"
                    ).last()
                    ...
                    try:
                        program_starting_date = datetime.datetime.strptime(
                            program_starting_date, "%d-%m-%Y"
                        ).date()
                    except ValueError as e:
                        # print("error in program starting date "+registation_no+" error is "+str(e))
                    ...
                    try:
                        program_completion_date = datetime.datetime.strptime(
                            program_completion_date, "%d-%m-%Y"
                        ).date()
                    except ValueError as e:
                        # print("program completion date "+registation_no+" error is "+str(e))
                    ...
                    if not thana_snd:
                        if not thana_snd_name:
                            ...
                            thana_snd = Thana(
                                name=thana_snd_name, district=district_snd
                            )
                            thana_snd.save()
                    ...
                    program_duration = ProgramDuration.objects.filter(
                    ...
                    if program_duration:
                        ...
                        program_duration = ProgramDuration(
                            duration=Decimal(program_length)
                        )
                        program_duration.save()

                    program_category = ProgramCatagory.objects.filter(
                        name__icontains=program_category_name
                    )
                    if program_category:
                    ...
                    if session_value:
                        session_value, start_date, end_date = get_month_year(
                            session_value
                        )
                    ...
                    ins_category = InstituteCatagory.objects.filter(
                        status__icontains=catagory
                    )
                    if ins_category:
                    ...
                    ins_type = InstituteType.objects.filter(
                        institute_type__icontains=type
                    )
                    if ins_type:
                        ins_type = ins_type[0]
                    else:
                    ...
                    if institution:
                        ...
                        institution = Institution(
                            ...
                            catagory=ins_category,
                            type=ins_type,
                            division=test_divisions,
                    ...
                    if ssc_edutype:
                        ...
                        ssc_education_qualification = EducationQualification(
                            ...
                            country=change_country_into_choice_field(ssc_country),
                        )
                    ...
                    if hsc_edutype:
                        ...
                        hsc_education_qualification = EducationQualification(
                            student=student_registration,
                            level_of_education=ssc_edutype,
                            education_type=" ",
                            board=hsc_college,
                            ...
                            country=change_country_into_choice_field(hsc_country),
                        )
                    ...
                    if diploma_edutype:
                        ...
                        diploma_education_qualification = EducationQualification(
                            student=student_registration,
                            ...
                            country=change_country_into_choice_field(diploma_country),
                        )
                    ...
                    if bsc_edutype:
                        ...
                        if bsc_gpa == "Pass":
                            ...
                            bsc_education_qualification = EducationQualification(
                                student=student_registration,
                                ...
                                country=change_country_into_choice_field(bsc_country),
                            )
                    ...
                    if msc_edutype:
                        ...
                        msc_education_qualification = EducationQualification(
                            student=student_registration,
                            ...
                            country=change_country_into_choice_field(msc_country),
                        )
                    ...
                    if doc_edutype:
                        ...
                        doc_education_qualification = EducationQualification(
                            student=student_registration,
                            ...
                            country=change_country_into_choice_field(doc_country),
                        )
        ...
        with open(pwd + "/accc.csv", encoding="utf8") as f:
            ...
            for row in reader:
                ...
                if not institution:
                    ...
                    if session_value:
                        session_value, session_start, sesson_end = get_month_year(
                            session_value
                        )
                        session = Session.objects.filter(session=session_value)
                        if session:
                    ...
                    created_institution = Institution(
                        institution_name=institution_name,
                        institution_code=institution_code,
                        catagory=ins_category,
                        type=ins_type,
...
def get_view(request):
    today = timezone.now()
    registation_list = Student_Registration.objects.filter(
        pk__in=(request.session.get("query_set"))
    )

    params = {"today": today, "request": request, "query_set": registation_list}
...
def process_licnese(request):
    if request.POST:
        ...
        if (
            ...
            license_students = license_registrations.objects.filter(
                rool_number__range=(start_role, end_role),
                approved=True,
                exam_title__id=exam,
            )
            ...
            create = CenterManagement.objects.create(
                exam_id=exam,
                roll_start=start_role,
                roll_end=end_role,
                center_id=center,
                hall_name=hall,
                room_name=room_no,
        ...
        if hidden_id:
            license_update_roll = license_registrations.objects.filter(
                rool_number__range=(start_role, end_role),
                approved=True,
                exam_title__id=exam,
            )
            update_query = license_update_roll.update(
                centre=center, hall_name=hall, room_name=room_no
            )
            query = CenterManagement.objects.filter(id=hidden_id)
            update = query.update(
                exam=exam,
    ...
    context = {
        "centers": centers,
        "exams": Exam.objects.filter(date_active=True).all(),
        "var": paginate,
    }
...
def renew_licence(request, license_id):
    ...
    renew = re_new_history(
        license=license_history.license_receive_id,
        created_on=datetime.datetime.now(),
        previous_start_date=license_history.license_start_date,
        previous_end_date=license_history.license_end_date,
        new_start_date=license_history.license_end_date,
        new_end_date=new_end_date,
        renew_by=request.user,
        license_number=license_history.license_number,
        program=license_history.program,
    )
    ...
    return redirect(
        "/admin/bnmc_project/license_receive/"
        + str(license_history.license_receive_id.id)
        + "/change/"
    )
...
def get_approve(request):
    ...
    if self_id:
        ...
        if last_query.IsApproved == False:
            update = query_srch.update(
                IsApproved=True,
                approvedBy=request.user.id,
                approvDate=datetime.datetime.now(),
            )
...
def student_money_recept(request):
        ...
        if request_type:
            ...
            if defaultAccounts:
                if request_type == "reg":
                    ...
                    student_registration = Student_Registration.objects.filter(
                        registration_no=reg_no, program_title__id=reg_program
                    )
                    if student_registration:
                        balance = BalanceHistory(
                            ...
                            historyType="1",
                            Note=note,
                            student_reg=student_registration[0],
                        )
                        balance.save()
                        total_amount = 0
                        ...
                        static_date = "11-03"
                        recept_number = 1
                        records = BalanceHistory.objects.filter(
                            create_on__year=datetime.datetime.now().year
                        )
                        if records:
                        ...
                        stu_reg = Student_Registration.objects.filter(
                            id=balance.student_reg.id
                        )
                        stu_reg = stu_reg[0]
                    ...
                    license_history = LicenseHistory.objects.filter(
                        license_number=license_reg_no, program=license_program
                    )
                    print()
                    note = request.POST.get("note")
                    balance = None
                        ...
                        if license_history.student_registration_id:
                            if license_history.student_registration_id:
                                balance = BalanceHistory(
                                    ...
                                    historyType="1",
                                    Note=note,
                                    student_reg=license_history.student_registration_id,
                                )
                                balance.save()
                                total_amount = 0
                                for income in Incomes:
                                    income_rec = BalanceIncome.objects.get(
                                        id=int(income)
                                    )
                                    if income_rec.amount:
                                        total_amount += income_rec.amount
                                ...
                                return render(
                                    request, "admin/money_recipte_2.html", contaxt
                                )
                            ...
                            student_id = Student.objects.filter(
                                id=license_history.student_id.id
                            )
                    ...
                    balance = BalanceHistory(
                        ...
                        historyType="1",
                        Note=note,
                        instituition=Institution.objects.get(id=institution),
                    )
                    text_list = []
                    item_text = []
                    ...
                    if balance and item_list:
                        contaxt = {
                            "p": balance,
                            "total_income": zip(
                                total_income, list_of, text_list, item_text
                            ),
    ...
    context = {
        "institution": Institution.objects.all(),
        "Incomes": BalanceIncome.objects.all().order_by("head_no"),
...
def re_store_stid(request):
    ...
    for std in students:
        html += (
            "<h1> id is " + str(std.id) + " student name is " + std.last_name + "</h1>"
        )
...
def edit_center(request):
    ...
    if center_query:
        info = [
            ...
            center_query.roll_end,
            center_query.center.id,
            center_query.hall_name,
            center_query.room_name,
        ]
        print(info)
    else:
        info = [False]
        ...
        with open(pwd + "/license_data.csv", encoding="utf8") as f:
            ...
            for row in reader:
                ...
                if reg != "" and program_code != "":
                    studentReg = Student_Registration.objects.filter(
                        registration_no=reg, program_title__code=program_code
                    )
                    if studentReg:
                        for i in studentReg:
        ...
        with open(pwd + "/missing_student_list.csv", encoding="utf8") as f:
            ...
            for row in reader:
                try:
                    ...
                    program_code = row["course_code"].strip()
                    file_signature = "signature_student/" + str(
                        row["signature"].strip()
                    )
                    exam_id = Exam.objects.get(id=1)
                    if reg != "" and program_code != "":
                        ...
                        if student_reg:
                            for i in student_reg:
                                license = license_registrations(
                                    student_registration=i,
                                    program=i.program_title,
                                    status=parmission,
                                    session=i.session,
                                    image=i.image,
                                    image_sec=i.image,
                                    students_mobile_no=i.students_mobile_no,
                                    signature_first=file_signature,
                                    exam_title=exam_id,
                                    date_of_passing_on=exam_id.exam_date,
    ...
    if request.method == "POST":
        ...
        with open(pwd + "/signature.csv", encoding="utf8") as f:
            ...
            for row in reader:
                ...
                file_signature = "signature_student/" + str(
                    row["Signatue_Name"].strip()
                )
                if reg != "" and program_code != "":
                    studentReg = license_registrations.objects.filter(
                        registration_no=reg, program__code=program_code
                    )
...
def found_pass(request):
    ...
    if filte:
        ...
        for i in filte:
            html += (
                "name "
                + str(i.student.last_name)
                + "  "
                + "program "
                + str(i.student.program_title)
                + "  "
                + "reg "
                + str(i.student.registration_no)
                + "<br>"
            )
