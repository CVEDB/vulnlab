...
class District(models.Model):
    division = models.ForeignKey(
        Division, verbose_name="Division Name", on_delete=models.CASCADE
    )
...
class Thana(models.Model):
    district = models.ForeignKey(
        District, verbose_name="District Name", on_delete=models.CASCADE
    )
...
class ProgramDuration(models.Model):
    duration = models.DecimalField(
        verbose_name="Program Duration", max_digits=10, decimal_places=2
    )
...
class ProgramCatagory(models.Model):
    name = models.CharField(
        verbose_name="Program Catagory Name", max_length=120, blank=False
    )
...
class Program(models.Model):
    category = models.ForeignKey(
        ProgramCatagory, verbose_name="Program Catagory ", on_delete=models.CASCADE
    )
    title = models.CharField(verbose_name="Program Name", max_length=100)
    code = models.CharField(
        verbose_name="Program Code", max_length=100, blank=True, null=True
    )
    second_code = models.CharField(max_length=100, blank=True, null=True)
    duration = models.ForeignKey(
        ProgramDuration, verbose_name="Program Duration", on_delete=models.CASCADE
    )
    program_fee = models.CharField(
        verbose_name="Program Fee", max_length=10, blank=True, null=True
    )
    session = models.ForeignKey(
        Session, blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    def save(self, *args, **kwargs):
        if not self._state.adding:
            qualification = Qualification.objects.filter(
                program_set__id=int(self.id)
            ).values_list("minimum_grade", flat=True)
            if qualification:
...
class Qualification(models.Model):
    minimum_qualification = models.CharField(
        verbose_name="Minimum Qualification", max_length=100
    )
    minimum_grade = models.FloatField(
        verbose_name="Minimum Grade", max_length=120, blank=True, null=True
    )
    program_set = models.ForeignKey(
        Program,
        blank=True,
        null=True,
        on_delete=models.CASCADE,
        verbose_name="program set",
    )
...
class Exam(models.Model):
    ...
    exam_code = models.CharField(
        verbose_name="Exam Code", max_length=50, blank=True, null=True
    )
...
class Institution(models.Model):
    ...
    catagory = models.ForeignKey(
        InstituteCatagory,
        verbose_name="Institution Catagory ",
        on_delete=models.CASCADE,
    )
    type = models.ForeignKey(
        InstituteType, verbose_name="Institution Type ", on_delete=models.CASCADE
    )
    ...
    principal_signature = models.FileField(
        verbose_name="Upload photo", upload_to="media/", null=True, blank=True
    )
...
class ExamCenter(models.Model):
    ...
    catagory = models.ForeignKey(
        InstituteCatagory, verbose_name="Center Catagory ", on_delete=models.CASCADE
    )
    type = models.ForeignKey(
        InstituteType, verbose_name="Center Type ", on_delete=models.CASCADE
    )
    division = models.ForeignKey(Division, on_delete=models.CASCADE)
...
class User(AbstractUser):
    ...
    staff_institute = models.ManyToManyField(
        Institution, blank=True, related_name="head_of"
    )
    show_permission = models.ManyToManyField(
        Permission, blank=False, related_name="show_permission_permissions"
    )
    staff_status = models.ManyToManyField(
        Permission, blank=False, related_name="staff_status_permissions"
    )
...
class SeatCapacity(models.Model):
    ...
    institiution = models.ForeignKey(
    ...
    def clean(self):
        if self._state.adding:
            queryset = SeatCapacity.objects.filter(
                institiution__id=self.institiution.id,
                session__id=self.session.id,
                program__id=self.program.id,
            )
            if queryset:
...
class Student(models.Model):
    def image_tag(self):
        return mark_safe(
            '<img  class="bc_img"  src="/media/%s" width="130" height="130"/>'
            % (self.image)
        )
    ...
    image = models.ImageField(
        verbose_name="Upload photo", upload_to="media/", blank=True
    )
    ...
    relation_to_guardians = models.ForeignKey(
        Relation_to_guardians, blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    division_snd = models.ForeignKey(
        Division,
    ...
    postal_code_snd = models.CharField(
        max_length=300, blank=True, verbose_name="Postal code", null=True
    )
...
class Student_Registration(models.Model):
    ...
    def image_tag(self):
        return mark_safe(
            '<img name="img" class="bc_im" src="/media/%s" width="130" height="130"/>'
            % (self.image)
        )
    ...
    first_name = models.CharField(
        verbose_name="Full name (Bangla)", max_length=300, null=True, blank=True
    )
    last_name = models.CharField(
        verbose_name="Full name (English)", max_length=300, null=True
    )
    fathers_name = models.CharField(max_length=300, null=True)
    mothers_name = models.CharField(max_length=300, null=True)
    guardians_name = models.CharField(max_length=300, blank=True, null=True)
    relation_to_guardians = models.ForeignKey(
        Relation_to_guardians, on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    district_snd = models.ForeignKey(
        District,
    ...
    post_office_snd = models.CharField(
        max_length=300, verbose_name="Post office", null=True
    )
    postal_code_snd = models.CharField(
        max_length=300, blank=True, verbose_name="Postal code", null=True
    )
    ...
    date_of_registration = models.DateField(
        verbose_name="Date of Admission", max_length=120, blank=True, null=True
    )
    ...
    bank_draft_no = models.CharField(
        verbose_name="Bank Draft No. ", max_length=300, blank=True, null=True
    )
    bank_draft_date = models.DateField(
        verbose_name="Bank Draft Date ", max_length=300, blank=True, null=True
    )
    ...
    status = models.ForeignKey(
        Permission, blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    migration = models.BooleanField(
        blank=True, default=False, verbose_name="migrateable"
    )
    migration_approval_bnmc = models.BooleanField(blank=True, default=False)
    new_institute = models.ForeignKey(
        Institution,
        on_delete=models.CASCADE,
    ...
    old_institute = models.ForeignKey(
        Institution,
        on_delete=models.CASCADE,
        related_name="old_intitution",
        blank=True,
        null=True,
    )
    ...
    approve_by = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.CASCADE, related_name="approve_by"
    )
    locked = models.BooleanField(default=False, blank=True)
    printed_by = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.CASCADE, related_name="printed_by"
    )
    ...
    def exam_link(self):
        if self.approved:
            return mark_safe(
                '<a href="/admin/bnmc_project/examinationstudentregistration/add/?student_registration=%d&program=%d&institute=%d">Attend In Exam<a/>'
                % (self.id, self.program_title.id, self.institution.id)
            )
        else:
            return ""
    ...
    def Apply_license_exam(self):
        if self.approved:
            return mark_safe(
                '<a href="/admin/bnmc_project/license_registrations/add?student_registration=%d&program_title=%d&registration_no=%s">Apply License Exam<a/>'
                % (self.id, self.program_title.id, self.registration_no)
            )
            ...
            if self.student_id:
                ...
                if student:
                    ...
                    student = Student(
                        ...
                        students_mobile_no=self.students_mobile_no,
                        division=self.division,
                        district=self.district,
                        ...
                        post_office=self.post_office,
                        village=self.village,
                        division_snd=self.division_snd,
                        district_snd=self.district_snd,
                        thana_snd=self.thana_snd,
                        village_snd=self.village_snd,
                        postal_code=self.postal_code,
                        post_office_snd=self.post_office_snd,
                        postal_code_snd=self.postal_code_snd,
    ...
    def clean(self):
        if self.session and self.program_title and self.institution:
            filter_for_seat = SeatCapacity.objects.filter(
                institiution__id=int(self.institution.id),
            ...
            if filter_for_seat:
                ...
                student_count_objects = Student_Registration.objects.filter(
                    session__id=int(self.session.id),
                    institution__id=int(self.institution.id),
                    program_title__id=int(self.program_title.id),
                ).count()

                if (
                    student_count_objects > filter_for_seat.seat
                ) or student_count_objects == filter_for_seat.seat:
...
class EducationQualification(models.Model):
    ...
    level_of_educations = models.ForeignKey(
    ...
    education_type = models.CharField(
        verbose_name="Education Type", max_length=120, choices=education_type_choice
    )
    board = models.CharField(
        max_length=120, verbose_name="University/Board", blank=True, null=True
    )
    roll = models.CharField(verbose_name="Roll", max_length=120, blank=True, null=True)
    cgpa = models.DecimalField(
        verbose_name="CGPA", max_digits=10, decimal_places=2, null=True
    )
    ...
    institution_name = models.ForeignKey(
        Institution, on_delete=None, blank=True, null=True
    )
...
class license_registrations(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    image = models.ImageField(
        verbose_name="Upload photo", upload_to="media/", blank=True, null=True
    )

    def image_second(self):
        return mark_safe(
            '<img name="img" class="bc_im_sec" src="/media/%s" width="130" height="130"/>'
            % (self.image_sec)
        )

    image_second.short_description = "Image 2"
    created_at = models.DateTimeField(auto_now_add=True)
    image_sec = models.ImageField(
        verbose_name="Upload photo", upload_to="media/", null=True, blank=False
    )
    rool_number = models.IntegerField(
        verbose_name="roll number", null=True, blank=True, default=0
    )
    session = models.ForeignKey(
        Session, max_length=120, on_delete=models.CASCADE, null=True
    )
    ...
    first_name = models.CharField(
        verbose_name="Full name (Bangla)", max_length=300, null=True
    )
    last_name = models.CharField(
        verbose_name="Full name (English)", max_length=300, blank=True, null=True
    )
    ...
    religions = models.CharField(
        max_length=120, choices=choice_religion, blank=True, null=True
    )
    ...
    district = models.ForeignKey(
        District, null=True, on_delete=models.CASCADE, blank=True
    )
    ...
    postal_code = models.CharField(max_length=300, blank=True, null=True)
    exam_title = models.ForeignKey(
        Exam, max_length=120, on_delete=models.CASCADE, null=True, blank=True
    )
    institution = models.ForeignKey(
        Institution,
        on_delete=models.CASCADE,
    ...
    def image_tag_li(self):
        return mark_safe(
            '<img class="bnc_s1" src="/media/%s" width="130" height="130"/>'
            % (self.signature_first)
        )

    image_tag_li.short_description = "Image"
    created_at = models.DateTimeField(auto_now_add=True)
    signature_first = models.ImageField(
        verbose_name="Upload photo",
        upload_to="signature_student/",
        blank=True,
        null=True,
    )
    ...
    def image_tag_lis(self):
        if self.institution:
            if self.institution.principal_signature:
                return mark_safe(
                    '<img width="143px" height="23px" src="%s"'
                    % self.institution.principal_signature.url
                )
        return None

    image_tag_lis.short_description = "Image"
    created_at = models.DateTimeField(auto_now_add=True)
    signature_sec = models.ImageField(
        verbose_name="up photo", upload_to="media/", blank=True, null=True
    )
    ...
    def print_licence(self):
        if self.approved:
            return mark_safe(
                '<a href="/print_license/%s"  target="_blank" >Print</a>' % (self.id)
            )
        else:
            return ""
    ...
    def license_receive_link(self):
        return mark_safe(
            '<a target="_blank" href="/admin/bnmc_project/license_receive/add/?id=%s">Registration for license</a>'
            % self.id
        )
    ...
    def print_licence_form(self):
        if self.approved:
            return mark_safe(
                '<a href="admin/print_license_form/"  target="_blank" >forms</a>'
                % (self.id)
            )
        else:
            return ""
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.student_registration:
            search_student_in_student_reg = Student_Registration.objects.get(
                id=self.student_registration.id
            )
            if search_student_in_student_reg.students:
                query_student = Student.objects.get(
                    id=search_student_in_student_reg.students.id
                )
                if query_student:
...
class license_receive(models.Model):
    ...
    def image_tag(self):
        return mark_safe(
            '<img src="/media/%s" width="130" height="130"/>' % (self.image)
        )
    ...
    student_id = models.IntegerField(null=True, blank=True, default=0)
    renew_history = models.ManyToManyField(
        "re_new_history", verbose_name="Renew History", blank=True
    )
    license_registrations_id = models.IntegerField(null=True, blank=True, default=0)
    license_registrations_refference = models.ForeignKey(
    ...
    approved = models.BooleanField(blank=True, default=False)
    license_registrations_receive_id = models.IntegerField(
    ...
    employer_country = models.CharField(
        max_length=120, choices=choice_country, null=True
    )

    division_snd = models.ForeignKey(
        Division,
        related_name="Division_per",
        on_delete=models.CASCADE,
        null=True,
        verbose_name="Division",
    )
    ...
    signature = models.ImageField(
        verbose_name="signature", upload_to="signature_student/", blank=True, null=True
    )

    def sig(self):
        return mark_safe(
            '<img  class="sig"  src="/media/%s" width="130" height="130"/>'
            % (self.signature)
        )
    ...
    program = models.ForeignKey(
        Program, max_length=120, on_delete=models.CASCADE, null=True, blank=True
    )
    institution = models.ForeignKey(
    ...
    permanent = models.BooleanField(
        blank=True, default=False, verbose_name="Permanent Address"
    )
    present = models.BooleanField(
        blank=True, default=False, verbose_name="Present Address"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.license_registrations_refference:
            ...
            exit_license_reg = license_registrations.objects.get(
                id=self.license_registrations_refference.id
            )
            if exit_license_reg:
                exit_student_reg = Student_Registration.objects.get(
                    id=exit_license_reg.student_registration.id
                )
                if exit_student_reg:
                    student_find = Student.objects.filter(
                        id=exit_student_reg.students.id
                    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.license_registrations_id or self.license_registrations_id != 0:
            license_reg = license_registrations.objects.filter(
                id=self.license_registrations_id
            )
    ...
    def license_form_print(self):
        return mark_safe(
            '<a target="_blank" href="/get_license_rec_stu/%s">print</a>' % (self.id)
        )
...
class re_new_history(models.Model):
    ...
    def delete(self, *args, **kwargs):
        ...
        if license_history:
            re = (
                re_new_history.objects.filter(
                    id__in=[thing.id for thing in license_history.renew_history.all()]
                )
                .exclude(id=self.pk)
                .last()
            )
...
class Job_History(models.Model):
    ...
    license_receive_reference = models.ForeignKey(
        "license_receive", null=True, blank=True, on_delete=models.CASCADE
    )
    license_registration_reference = models.ForeignKey(
        "license_registrations", null=True, blank=True, on_delete=models.CASCADE
    )

    current_designation = models.CharField(max_length=50, blank=False, null=True)
    current_hospital = models.ForeignKey(
        "Hospital", blank=False, null=True, on_delete=models.CASCADE
    )
...
class ExamSubject(models.Model):
    ...
    SubSubjects = models.ManyToManyField(
        "ExamSubject", verbose_name="Sub Subjects", blank=True
    )
...
class SubSubject(models.Model):
    subject_name = models.ForeignKey(
        SubSubjectName, on_delete=models.SET_NULL, null=True
    )
...
class ExaminationStudentRegistration(models.Model):
    ...
    exam = models.ForeignKey(
        Final_exam, on_delete=models.CASCADE, null=False, blank=False
    )
    year = models.ForeignKey(ExamYear, on_delete=None, null=True)

    student_registration = models.ForeignKey(
        Student_Registration, on_delete=None, blank=False
    )
    ...
    student_signature = models.ImageField(
        verbose_name="Candidate Signature", upload_to="signature_student/", blank=True
    )
    approved = models.BooleanField(blank=True, default=False)
    ...
    center = models.ForeignKey(
        Institution, null=True, on_delete=None, related_name="Center"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            if self.student_registration:
                exit_student = Student.objects.get(
                    id=self.student_registration.students.id
                )
                if exit_student:
...
class Examination_result_add(models.Model):
    exam_id = models.ForeignKey(
        ExaminationStudentRegistration, on_delete=models.CASCADE
    )
...
class ExamResultDetails(models.Model):
    examStudentInfo = models.ForeignKey(
        ExaminationStudentRegistration,
        on_delete=models.CASCADE,
        null=False,
        blank=False,
    )
    subject = models.ForeignKey(
        ExamSubject, on_delete=models.CASCADE, null=False, blank=False
    )
    marks = models.FloatField(null=True, blank=True, default=0.000)
        ...
        if self.subject.passMarks and self.marks:
            ...
            if self.subject.isMainSubject:
                if self.subject.SubSubjects:
                    subjectIds = [
                        item["id"]
                        for item in self.subject.SubSubjects.all().values("id")
                    ]
                    subsubjects = ExamResultDetails.objects.filter(
                        examStudentInfo=self.examStudentInfo, subject__pk__in=subjectIds
                    )
...
class BalanceHistory(models.Model):
    ...
    historyType = models.CharField(
        choices=choice_historyType, blank=False, max_length=200
    )
    ...
    TransfarRaffer = models.ForeignKey(
        "TransfarBalance", on_delete=None, blank=True, null=True
    )
    create_on = models.DateField(null=True, blank=True)
    edit_on = models.DateTimeField(null=True, blank=True, auto_now=True)
    BalanceIncomes = models.ManyToManyField("BalanceIncome", blank=True)
    student_reg = models.ForeignKey(
        Student_Registration, on_delete=None, blank=True, null=True
    )
    ...
    bankIssueDate = models.DateField(
        ("Date"), null=True, blank=True, default=date.today
    )
    add = 1
    remove = 2
    ...
    def getTransfarLocation(self):
        if self.IsTransfardBalance:
            if self.TransfarRaffer:
                return mark_safe(
                    "<a href='/admin/bnmc_project/transfarbalance/"
                    + str(self.TransfarRaffer.id)
                    + "/change/'>"
                    + str(self.TransfarRaffer)
                    + "</a>"
                )
        return ""
    ...
    def get_add_or_remove(self):
        if self.historyType == "1":
            return mark_safe(
                "<span style='width: 10px;height: 10px;background-color: green;border-radius: 100%;content: "
                ";display: block;margin-left: 36px;'></span>"
            )

        if self.historyType == "2":
            return mark_safe(
                "<span style='width: 10px;height: 10px;background-color: red;border-radius: 100%;content: "
                ";display: block;margin-left: 36px;'></span>"
            )
        return ""
...
class LicenseHistory(models.Model):
    ...
    registration_no = models.CharField(
        null=True, blank=True, max_length=300, verbose_name="Student id"
    )
    institution = models.ForeignKey(
        Institution, on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    money_recipte_id = models.ForeignKey(
        "BalanceHistory", on_delete=models.CASCADE, null=True, blank=True
    )
    date_of_passing = models.CharField(max_length=120, blank=True, null=True)
    license_receive_id = models.ForeignKey(
        "license_receive", null=True, blank=True, on_delete=models.CASCADE
    )
    student_id = models.ForeignKey(
        "Student", null=True, blank=True, on_delete=models.CASCADE
    )
    student_registration_id = models.ForeignKey(
        "Student_Registration", null=True, blank=True, on_delete=models.CASCADE
    )
    license_registration_id = models.ForeignKey(
        "license_registrations", null=True, blank=True, on_delete=models.CASCADE
    )
    renew_history = models.ManyToManyField(
        "re_new_history", verbose_name="Renew History", blank=True
...
class TransfarBalance(models.Model):
    FromAccount = models.ForeignKey(
        Accounts, null=False, blank=False, on_delete=None, related_name="FromAccount"
    )
    ToAccount = models.ForeignKey(
        Accounts, null=False, blank=False, on_delete=None, related_name="ToAccount"
    )
    ...
    def save(self, *args, **kwargs):
        if self._state.adding:
            ...
            self.edit_on = datetime.datetime.now()
        super().save(*args, **kwargs)
        removeBalance = BalanceHistory(
            account=self.FromAccount,
            historyType=2,
            amount=self.amount,
            Note="Transfar Id id " + str(self.id),
            IsTransfardBalance=True,
            TransfarRaffer=self,
            IsApproved=True,
        )
        removeBalance.save(commit=False)
        addBalance = BalanceHistory(
            account=self.ToAccount,
            historyType=1,
            amount=self.amount,
            Note="Transfar Id id " + str(self.id),
            IsTransfardBalance=True,
...
class Student_file(models.Model):
    student_file_program = models.ForeignKey(
        StudentFileProgram, on_delete=models.CASCADE, null=True
    )
    student_registration = models.ForeignKey(
        Student_Registration, blank=True, null=True, on_delete=models.CASCADE
    )
...
class IntuitionProfile(models.Model):
    ...
    def Image_View(self):
        return mark_safe(
            '<img  class="cb_img"  src="/media/%s" width="130" height="130"/>'
            % (self.image)
        )
    ...
    employment_type = models.CharField(
        max_length=300, blank=True, null=True, choices=type_choices, default=2
    )
...
class ApplyLicense(models.Model):
    ...
    currently_working_place = models.CharField(
        max_length=300, blank=True, null=True, choices=choice_type
    )
    current_position = models.CharField(max_length=300, blank=True, null=True)
    Working_years_as_a_registered = models.CharField(
        max_length=300, blank=True, null=True
    )
    Training = models.CharField(max_length=300, blank=True, null=True)
    Attendance_in_Seminar = models.CharField(
        verbose_name="Attendance in Seminar/ Symposium/Workshop/Case Presentation/ Meeting",
        max_length=300,
        blank=True,
        null=True,
    )
    Research_skills = models.CharField(
        verbose_name="Research Publications/ Other skills",
        max_length=300,
        blank=True,
        null=True,
    )
    license_id = models.ForeignKey(
        license_receive, on_delete=None, null=True, blank=True
    )
    I_am_satisfied = models.BooleanField(
        blank=True, default=False, verbose_name="I am satisfied with my Job"
    )
    before_starting = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="Before starting my work I exchange my greetings with the patients",
    )
    I_own_my = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="I own my responsibility as one of the  Medical Team Members",
    )
    I_am_interested = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="I am interested  to act as a nurse teacher in my working situation",
    )
    I_am_aware = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="I am aware of  my  working area, delegated authority & responsibilities",
    )
    I_am_aware_of_prescribed = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="I am aware of  prescribed code of conduct & ethics of Bangladesh Nursing Council",
    )
    In_Bangladesh_most = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="In Bangladesh most of the basic nursing activities are performed by patients’ attendance",
    )
    Every_day = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="Every day I do my work through nursing care plan",
    )
    I_always_try = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="I always try my best level to treat the patients equally in terms of race, religion, gender, age, nationality & economical class",
    )
    I_am_aware_of_functions = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="I am aware of  functions of Bangladesh Nursing Council",
    )
    I_always_wear_my_professional = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="I always wear my professional Uniform as approved by the Govt. of Bangladesh",
    )
    I_feel_the_present = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="I feel the present existing system of Registration & Renewal should be digitalized",
    )
    I_feel_there_should_be = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="I feel there should be provision of examination system for Registration Renewal in Bangladesh like other countries",
    )
    I_know_the_number = models.BooleanField(
        blank=True,
        default=False,
        verbose_name="I know the  number of registered nurses, nursing institutions & colleges",
    )
...
class RequestedLicense(models.Model):
    ...
    license_number = models.CharField(max_length=120, blank=True, null=True)
    applyLicense = models.ForeignKey(
        ApplyLicense, on_delete=None, null=True, blank=True
    )
