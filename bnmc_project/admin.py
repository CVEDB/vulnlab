    ...
    def get_formset(self, request, obj=None, **kwargs):
        ...
        if request.method == "GET":
            if request.GET.get("program_title", None):
                student_file = StudentFileProgram.objects.filter(
                    program_id__id=request.GET.get("program_title", None)
                )
                if student_file:
        ...
        if request.method == "GET":
            if request.GET.get("program_title", None):
                qualifications = Qualification.objects.filter(
                    program_set__id=request.GET.get("program_title", None)
                )
                if qualifications:
...
class RegisterAdmin(admin.ModelAdmin):
    ...
    def get_last_name(self, obj):
        if obj.students:
            return mark_safe(
                '<a href="/admin/bnmc_project/student_registration/%s/change/">%s</a>'
                % (obj.id, obj.students.last_name)
            )

        else:
            return mark_safe(
                '<a href="/admin/bnmc_project/student_registration/%s/change/">%s</a>'
                % (obj.id, obj.last_name)
            )
    ...
    def get_result(self, obj):
        if obj.id:
            final_exam_reg = ExaminationStudentRegistration.objects.filter(
                student_registration=obj.id
            )
            for fin in final_exam_reg:
                if fin.student_registration:
                    final_exam_result = ExamResultDetails.objects.filter(
                        examStudentInfo=int(fin.id)
                    )
    ...
    def add_view(self, request, form_url="", extra_context=None):
        ...
        if program_id:
            ...
            extra_context = {"var": pro_q.category.name}
            if request.GET.get("source", None):
                if int(request.GET.get("source", None)) > 0:
                    license_rec = license_receive.objects.get(
                        id=request.GET.get("source", None)
                    )
                    if license_rec.is_old_data == False:
                        license_reg = license_registrations.objects.get(
                            id=license_rec.license_registrations_refference.id
                        )
                        if license_reg:
                            student_reg = Student_Registration.objects.get(
                                id=license_reg.student_registration.id
                            )
                            if student_reg:
                                student_data = Student.objects.get(
                                    id=student_reg.students.id
                                )
                                if student_data:
                                    ...
                                    request.GET[
                                        "post_office"
                                    ] = student_data.post_office
                                    request.GET[
                                        "postal_code"
                                    ] = student_data.postal_code
                                    request.GET["institution"] = request.GET.get(
                                        "institution"
                                    )
                                    request.GET["program_title"] = request.GET.get(
                                        "program_title"
                                    )
                                    request.GET[
                                        "registration_no"
                                    ...
                                    request.GET[
                                        "students_mobile_no"
                                    ] = student_data.students_mobile_no
                                    request.GET[
                                        "division_snd"
                                    ] = student_data.division_snd
                                    request.GET[
                                        "district_snd"
                                    ] = student_data.district_snd
                                    request.GET["thana_snd"] = student_data.thana_snd
                                    request.GET[
                                        "village_snd"
                                    ] = student_data.village_snd
                                    request.GET[
                                        "post_office_snd"
                                    ] = student_data.post_office_snd
                                    request.GET[
                                        "postal_code_snd"
                                    ] = student_data.postal_code_snd
                        ...
                        request.GET[
                            "national_ID_No"
                        ] = license_rec.students.national_ID_No
                        ...
                        request.GET[
                            "registration_no"
                        ] = license_rec.students.registration_no
                        ...
                        request.GET[
                            "students_mobile_no"
                        ] = license_rec.students.students_mobile_no
                        ...
                        request.GET[
                            "post_office_snd"
                        ] = license_rec.students.post_office_snd
                        request.GET[
                            "postal_code_snd"
                        ] = license_rec.students.postal_code_snd
    ...
    def changeform_view(self, request, object_id=None, form_url="", extra_context=None):
        ...
        if object_id is not None:
            ...
            if (
                student.status == last_permission
                and request.user.can_modify_after_approved == False
            ):
                ...
                student.status == last_permission
                and request.user.can_modify_after_approved == True
    ...
    def approve_to_draft(self, request, queryset):
        ...
        queryset.update(
            approve_by="", approved=False, registration_no="", status=first_permission
        )
        ...
        for reg_std in queryset:
            if reg_std.status and reg_std.status != permission_query_set.last():
                ...
                if len(premissions) > current_index - 1:
                    ...
                    if (
                        reg_std.status == permission_query_set.last()
                        and reg_std.approved == False
                    ):
                        reg_no = 1
                        if program_short_name.get(reg_std.program_title.code, None):
                            reg_no = (
                                Student_Registration.objects.filter(
                                    program_title__code=reg_std.program_title.code,
                                    approved=True,
                                )
                                .extra(
                                    {
                                        "registration_no_int": "CAST(registration_no as INTEGER)"
                                    }
                                )
                                .order_by("registration_no_int")
                                .last()
                            )
    ...
    def select_print(self, request, queryset):
        ...
        context = {"students": zip(student_query, queryset), "edu": edu}
    ...
    def get_queryset(self, request):
        if request.user.is_superuser:
            if request.POST:
                # last_permission = Permission.objects.order_by('display_order').last()
                student_registations = Student_Registration.objects.filter(
                    status__id__in=request.user.show_permission.values_list("id")
                )
                if (
                    request.POST.get("ins_type")
                    and int(request.POST.get("ins_type")) > 0
                ):
                    ins_type = request.POST.get("ins_type")
                    student_registations = student_registations.filter(
                        institution__type__id=ins_type
                    )
                    print(student_registations)
                    # return Student_Registration.objects.filter(Q(institution=ins_type))
                if (
                    request.POST.get("progmType")
                    and int(request.POST.get("progmType")) > 0
                ):
                    progmType = request.POST.get("progmType")
                    student_registations = student_registations.filter(
                        program_title=progmType
                    )
                ...
                if request.POST.get("ins_cat") and int(request.POST.get("ins_cat")) > 0:
                    ...
                    student_registations = student_registations.filter(
                        institution__catagory=ins_cat
                    )
                if (
                    request.POST.get("program_category")
                    and int(request.POST.get("program_category")) > 0
                ):
                    program_category = request.POST.get("program_category")
                    student_registations = student_registations.filter(
                        program_title__category=program_category
                    )
                if (
                    request.POST.get("ins_name")
                    and int(request.POST.get("ins_name")) > 0
                ):
        ...
        if (
            ...
            and request.user.show_permission
            and request.user.staff_institute
        ):
            if request.POST:
                student_registations = Student_Registration.objects.filter(
                ...
                if (
                    ...
                    program_category = request.POST.get("program_category")
                    student_registations = student_registations.filter(
                        program_title__category__id=program_category
                    )
        ...
        if request.user.is_superuser:
            ...
            and request.user.show_permission
            and request.user.staff_institute
            and request.GET.get("license_use", None)
        ):
            return Student_Registration.objects.filter(
                institution__id__in=request.user.staff_institute.values_list("id"),
                approved=True,
            ...
            and request.user.show_permission
            and request.user.staff_institute
            and request.GET.get("approved", None)
        ):
            return Student_Registration.objects.filter(
                institution__id__in=request.user.staff_institute.values_list("id"),
                approved=True,
...
class Job_history_inline_in_license_registration(admin.TabularInline):
    ...
    exclude = (
        "student",
        "license_receive_reference",
    )


class student_admin(admin.ModelAdmin):
    inlines = [
        Education_qualificationTabularInline,
    ]
...
class user_search(admin.ModelAdmin):
    ...
    fields = (
        ...
        "is_superuser",
        "is_bnmc_user",
        "allow_for_old_license_add",
        "is_result",
        "can_print_registration_form",
        "can_modify_after_approved",
        "first_name",
        "last_name",
        "email",
        "staff_institute",
...
class InstituteFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if (
            request.GET.get("institute_id")
            and not request.GET.get("programreal_id")
            and not request.GET.get("no")
        ):
            print("prooooooooooooo")

            li = LicenseHistory.objects.filter(
...
class License_number_filter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if request.GET.get("programreal_id") and request.GET.get("no"):
            print("two")
            li = LicenseHistory.objects.filter(
                license_number=request.GET.get("no"),
                program=request.GET.get("programreal_id"),
            )
            ...
            request.GET.get("no")
            and request.GET.get("institute_id")
            and request.GET.get("programreal_id")
        ):
            li = LicenseHistory.objects.filter(
                license_number=request.GET.get("no"),
                institution=request.GET.get("institute_id"),
...
class license_receive_admin(admin.ModelAdmin):
    ...
    inlines = [
        license_receive_inline,
        license_history_admin_inline,
        working_inline,
        t_admin,
    ]
    ...
    fields = (
        ...
        "institution",
        "late",
        "license_registrations_refference",
    )
    ...
    def get_queryset(self, request):
        ...
        if (
            ...
            return license_receive.objects.filter(
                status__id__in=request.user.show_permission.values_list("id"),
                institution__id__in=request.user.staff_institute.values_list("id"),
            )
    ...
    def response_add(self, request, obj, post_url_continue=None):
        if request.GET.get("id"):
            ...
            if license_id:
                stu_status = Student_Registration.objects.get(
                    id=license_id.student_registration.id
                )
    ...
    def Approved(self, request, queryset):
        for record in queryset:
            ...
            while not validate:
                print(record)
                exist_record = license_receive.objects.filter(
                    license_registrations_receive_id=reg_number
                )
                if exist_record:
                    reg_number += 1
                else:
    ...
    def add_view(self, request, form_url="", extra_context=None):
        if request.GET.get("id", None):
            ...
            student_reg = Student.objects.get(
                pk=license_reg.student_registration.students.id
            )
...
class license(admin.ModelAdmin):
    inlines = [
        Job_history_inline_in_license_registration,
    ]
    ...
    def get_last_name(self, obj):
        if obj.student_registration:
            student_id = Student_Registration.objects.get(
                id=obj.student_registration.id
            )
            if student_id.students:
                query_student = Student.objects.get(id=student_id.students.id)
                if query_student.last_name:
                    return mark_safe(
                        '<a href="/admin/bnmc_project/license_registrations/%s/change/">%s</a>'
                        % (obj.id, query_student.last_name)
                    )
                else:
                    return None
            else:
    ...
    def add_view(self, request, form_url="", extra_context=None):
        if request.GET.get("registration_no", None):
            ...
            try:
                ...
                program_id = 0
            student_infos = Student_Registration.objects.filter(
                registration_no=reg_number, program_title__id=program_id
            )
            ...
            if reg_std.status and reg_std.status != permission_query_set.last():
                ...
                if len(premissions) > current_index - 1:
                    ...
                    if (
                        ...
                        if program_short_name.get(reg_std.program.code, None):
                            reg_no = (
                                license_registrations.objects.filter(
                                    program__code=reg_std.program.code, approved=True
                                )
                                .extra(
                                    {"rool_number_int": "CAST(rool_number as INTEGER)"}
                                )
                                .order_by("rool_number_int")
                                .last()
                            )
                        ...
                        while is_valid == False:
                            auto_reg_no = license_registrations.objects.filter(
                                program__code=reg_std.program.code, rool_number=reg_no
                            )
                            if auto_reg_no:
                                reg_no += 1
                            else:
    ...
    def get_queryset(self, request):
        ...
        if request.GET.get("rendom", None):
            ...
            return license_registrations.objects.filter(
                program=request.GET.get("program__id__exact", None),
                status=last_permission,
                institution__id__in=request.user.staff_institute.values_list("id"),
            ).order_by("?")

        if request.user.is_superuser and request.GET.get("received_filter", None):
            licence_regs = licence_regs.filter(
                exam_title__pass_mark__lte=F("pass_mark"), approved=True
            )
            return licence_regs
            print("super pere")
        ...
        if (
            request.user.is_staff
            and request.GET.get("received_filter", None)
            and not request.user.is_superuser
        ):
            licence_regs = licence_regs.filter(
                exam_title__pass_mark__lte=F("pass_mark"),
                approved=True,
                institution__id__in=request.user.staff_institute.values_list(
                    "id"
                ).all(),
            )
            print("ooo")
...
class District_admin(admin.ModelAdmin):
    search_fields = ("name",)
    list_filter = (("division", RelatedDropdownFilter), DistrictListFilter)
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        if db_field.name == "SubSubjects":
            kwargs["queryset"] = ExamSubject.objects.filter(
                isMainSubject=False, SubSubjects=None
            )
...
class ExaminationRegistrationAdmin(admin.ModelAdmin):
            ...
            if reg_std.status and reg_std.status != permission_query_set.last():
                ...
                if len(premissions) > current_index - 1:
                    ...
                    if (
                        ...
                        if program_short_name.get(reg_std.program.code, None):
                            reg_no = (
                                ExaminationStudentRegistration.objects.filter(
                                    program__code=reg_std.program.code, approved=True
                                )
                                .extra(
                                    {"roll_number_int": "CAST(roll_number as INTEGER)"}
                                )
    ...
    def response_add(self, request, obj, post_url_continue=None):
        context = {"p": obj, "sub": obj.subjects.all()}
        return render(request, "admin/comprehenshive_exam_form.html", context)
    ...
    def get_queryset(self, request):
        ...
        if request.user.is_superuser:
            ...
            request.user.is_staff
            and request.user.result_institute
            and request.user.result_for_program
            and request.user.is_result
        ):
            return ExaminationStudentRegistration.objects.filter(
                institute__id__in=request.user.result_institute.values_list("id"),
                program__id__in=request.user.result_for_program.values_list("id"),
            )

        elif request.user.is_staff and request.user.staff_institute:
            return ExaminationStudentRegistration.objects.filter(
                institute__id__in=request.user.staff_institute.values_list("id")
            )
    ...
    def add_view(self, request, form_url="", extra_context=None):
        ...
        if request.GET.get("student_registration", None):
            student_reg = Student_Registration.objects.get(
                id=request.GET.get("student_registration", None)
            )
            if student_reg:
...
class BalanceHistoryAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        if removedBalances:
            removedAmounts = reduce(
                lambda x, y: x + y,
                removedBalances.filter(amount__isnull=False, amount__gt=1)
                .all()
        ...
        if addedBalances:
            addedAmountsObjects = (
                addedBalances.filter(amount__isnull=False, amount__gt=1)
                .all()
                .values_list("amount", flat=True)
            )
...
class institute_profile_admin(admin.ModelAdmin):
    ...
    def add_view(self, request, form_url="", extra_context=None):
        if request.GET.get("registration_no", None):
            if not request.GET._mutable:
                request.GET._mutable = True
            license_history_found = LicenseHistory.objects.filter(
                license_number=request.GET.get("registration_no")
            )[0]
            if license_history_found.license_receive_id.is_old_data:
                request.GET[
                    "full_name_english"
                ] = license_history_found.license_receive_id.students.last_name
                request.GET[
                    "father_name"
                ] = license_history_found.license_receive_id.students.fathers_name
                request.GET[
                    "sex"
                ] = license_history_found.license_receive_id.students.sex
                request.GET[
                    "mother_name"
                ] = license_history_found.license_receive_id.students.mothers_name
                request.GET[
                    "date_of_birth"
                ] = license_history_found.license_receive_id.students.date_of_birth
                request.GET[
                    "marital_status"
                ] = license_history_found.license_receive_id.students.marital_status
                request.GET[
                    "national_ID_No"
                ] = license_history_found.license_receive_id.students.national_ID_No
                request.GET[
                    "email_address"
                ] = license_history_found.license_receive_id.students.email_address
                request.GET[
                    "division"
                ] = license_history_found.license_receive_id.students.division
                request.GET[
                    "district"
                ] = license_history_found.license_receive_id.students.district
                request.GET[
                    "thana"
                ] = license_history_found.license_receive_id.students.thana
                request.GET[
                    "village"
                ] = license_history_found.license_receive_id.students.village
                request.GET[
                    "post_office"
                ] = license_history_found.license_receive_id.students.post_office
                request.GET[
                    "postal_code"
                ] = license_history_found.license_receive_id.students.postal_code
                request.GET[
                    "nationality"
                ] = license_history_found.license_receive_id.students.nationality
                request.GET[
                    "student"
                ] = license_history_found.license_receive_id.students
                ...
                if license_history_found:
                    license_reg_id = license_registrations.objects.get(
                        id=license_history_found.license_registration_id.id
                    )
                    if license_reg_id:
                        student_reg = Student_Registration.objects.get(
                            id=license_reg_id.student_registration.id
                        )
                        if student_reg:
    ...
    def get_queryset(self, request):
        if request.user.is_staff:
            return IntuitionProfile.objects.filter(
                institute_name__id__in=request.user.staff_institute.values_list("id")
            )
