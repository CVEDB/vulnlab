...
class ReconpointUpdateCheck(APIView):
    def get(self, request):
        req = self.request
        github_api = "https://api.github.com/repos/khulnasoft/reconpoint/releases"
        ...
        current_version = (
            (os.environ["RECONPOINT_CURRENT_VERSION"])[1:]
            if os.environ["RECONPOINT_CURRENT_VERSION"][0] == "v"
            else os.environ["RECONPOINT_CURRENT_VERSION"]
        ).replace("'", "")
    ...
    def get(self, request, format=None):
        ...
        if "nuclei_template" in req.query_params:
            ...
            if is_safe_path(safe_dir, path) and os.path.exists(path):
                content = open(path.format(name)).read()
                response["status"] = True
                response["content"] = content
            else:
                response["message"] = "Invalid Path!"
...
class EndPointViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        endpoints_obj = EndPoint.objects.filter(
